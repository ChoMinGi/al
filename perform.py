...
def run_test_reduced(graph_type, sizes, iterations):
    results = []
    for n in sizes:
        kruskal_times, prim_with_fib_heap_times = [], []
        for _ in range(iterations):
            ...
            start = time.time()
            prim_with_fib_heap(n, graph)
            prim_with_fib_heap_times.append(time.time() - start)
        ...
        results.append({
            'n': n,
            'prim_early_avg_time': sum(prim_with_fib_heap_times) / iterations,
